...
export default function RestoreGateModal({ restoreData, onRestoreSuccess, onStartNew }: Props) {
  ...
  async function handleSendCode() {
    ...
    if (email.toLowerCase().trim() !== restoreData.email?.toLowerCase().trim()) {
      setError('This email does not match the one used to save this proposal.')
      return
    }
    ...
    try {
      const res = await fetch('/api/auth/send-otp', {
        ...
        body: JSON.stringify({
          email: email.trim(),
          proposalId: restoreData.proposalId,
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
        <div className="relative w-full max-w-sm bg-[#1a1a1a] border border-white/10 rounded-2xl shadow-2xl p-6 space-y-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-semibold text-white">Proposal not saved</h2>
          </div>

          <p className="text-sm text-[#727272] leading-relaxed">
            This proposal was started on another device and was not saved. Use &ldquo;Save for Later&rdquo; next time to access your proposals from any device.
          </p>
          ...
          >
            Start a new proposal <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
    )
        ...
        {step === 'email' && (
          <>
            <div>
              ...
              <p className="text-sm text-[#727272] leading-relaxed">
                Enter the email you used when saving this proposal. We'll send a code to verify it's you.
              </p>
            </div>
